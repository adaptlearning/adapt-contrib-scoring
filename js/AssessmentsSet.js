...
export default class AssessmentsSet extends ScoringSet {
  ...
  /**
   * @private
   * @todo `_assessments` rather than `assessment`
   */
  _initConfig() {
    this._config = Adapt.course.get('_assessments');
  ...
  /**
   * Reset all subsets
   * @todo Trigger an event to say all assessments have been reset?
   */
  reset() {
   ...
   * @returns {Boolean}
   */
  getModelHasAssessment(model) {
    return hasIntersectingHierarchy([model], this.models);
    //return this.subsets.find(set => set.model === model);
  }

  /**
   * Returns an assessment associated with a specified model id
   * @todo If more than one associated assessment it will return the first found
   * @param {String} id
   * @returns {AssessmentSet}
   */
  getByModelId(id) {
    return getSubsetsByModelId(id).find(set => set.type === 'assessment');
  }

  /**
   * Returns the assessment with the specified id
   * @param {String} id
   * @returns {AssessmentSet}
   */
  getById(id) {
