...
export default class AssessmentSet extends ScoringSet {
   ...
   */
  _initConfig() {
    this._config = this.model.get('_assessment');
  ...
  /**
   * Returns the model containing the `_assessment` config
   * @returns {AdaptModel}
   */
  get model() {
    return this._model;
   ...
   */
  get minScore() {
    if (this.isComplete && !Boolean(this.attempt?.isInSession)) {
  ...
  /**
   * Returns whether the assessment can be reset
   * @returns {Boolean}
   */
  get canReset() {
   ...
   * A previously completed "soft" reset assessment will be deemed completed.
  ...
  get isComplete() {
    if (!this.isAllModelsAdded) return false;
    return Boolean(this.attempt?.isInSession) ? this.isAttemptComplete : this.attempts.history.some(attempt => attempt.isComplete);
  ...
  get isPassed() {
    ...
    if (!Boolean(this.attempt?.isInSession)) return this.attempts.history.some(attempt => attempt.isPassed);
  ...
  async onRouterLocation(location) {
    ...
    if (!(Adapt.assessments.getModelHasAssessment(model))) return;
